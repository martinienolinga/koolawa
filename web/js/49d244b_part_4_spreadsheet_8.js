...
function initSpreadBox(sheetnumber)
{
	...
	/*globalSpreadbox[sheetnumber].tabfeuilleBody.on('resize', function(e){
		document.title = $('#spreadbox_' + sheetnumber).parent().height();
	});*/
	
	//alert(Ext.getCmp('tabFeuille').title);
	
	//var tabFeuilleID = globalSpreadbox[sheetnumber].tabfeuilleBody.parent().id;
	//alert(Ext.getCmp(tabFeuilleID).title);
	   ...
	   for(i = 0; i < columns; i++)
	   {
	      ...
	      resizecolumnbox.on('mousedown', function(e) {
	         if (globalSpreadbox[sheetnumber].resizecolumnboxDown == null)
	         {
	            ...
	            globalSpreadbox[sheetnumber].sizeableVrtdiv.height(globalSpreadbox[sheetnumber].tabfeuilleBody.height());
	      ...
	      $('#sheet_' + sheetnumber + ' .row').each(function(index)
	      {
	         ...
	         cell.attr('address', index + " " + cindex); //$(this).find('.cell').size());
	...
	globalSpreadbox[sheetnumber].getVisiblesCells = function(sheetnumber) {
		...
		for (var i = 0; i < rows.length; i++) {
			...
			// on exclu toutes les lignes extérieures à la zone visible
			if (rows[i].offsetTop - globalSpreadbox[sheetnumber].spreadboxEl.offsetTop + rows[i].offsetHeight < globalSpreadbox[sheetnumber].sheetEl.scrollTop) continue;
			if (rows[i].offsetTop - globalSpreadbox[sheetnumber].spreadboxEl.offsetTop + rows[i].offsetHeight > globalSpreadbox[sheetnumber].sheetEl.scrollTop + tabFeuilleBody.offsetHeight + 20) continue;

			collectedJqueryRows.push($(rows[i]));
		}
		...
		for (var i = 0; i < collectedJqueryRows.length; i++) {
			...
			for (var c = 0; c < cellsx.length; c++) {
				...
				// on exclu toutes les cellules extérieures à la zone visible
				if (cellsx[c].offsetLeft - globalSpreadbox[sheetnumber].spreadboxEl.offsetLeft + cellsx[c].offsetWidth < globalSpreadbox[sheetnumber].sheetEl.scrollLeft) continue;
				if (cellsx[c].offsetLeft - globalSpreadbox[sheetnumber].spreadboxEl.offsetLeft + cellsx[c].offsetWidth > globalSpreadbox[sheetnumber].sheetEl.scrollLeft + tabFeuilleBody.offsetWidth + 50) continue;
				
				collectedCells.push(cellsx[c]);
			}
		}
...
function resizeSpreadBox(sheetnumber)
{
	...
	var parentHeight = parentBody.height() - 20;
	...
	if (typeof globalSpreadbox[sheetnumber].sizeableVrtdiv !== 'undefined') globalSpreadbox[sheetnumber].sizeableVrtdiv.height(globalSpreadbox[sheetnumber].tabfeuilleBody.height());
